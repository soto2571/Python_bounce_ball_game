...
BALL_RADIUS = 20
PLATAFORM_WIDTH, PLATAFORM_HEIGHT = 100, 10
FPS = 60
BLACK = (0,0,0)
WHITE = (255,255,255)
RED = (255,0,0)
...
while game_running:
    ...
    platform_pos[0] = max(0, min(platform_pos[0], WIDTH - PLATAFORM_WIDTH))
    platform_pos[1] = max(0, min(platform_pos[1], HEIGHT - PLATAFORM_HEIGHT))
    ...
    if (
        platform_pos[0] <= ball_pos[0] <= platform_pos[0] + PLATAFORM_WIDTH
        and platform_pos[1] <= ball_pos[1] <= platform_pos[1] + PLATAFORM_HEIGHT
        ):
    ...
    # Check if the ball fall off the screen
    if ball_pos[1] >= HEIGHT:
        # Decrese lives
        lives -= 1
        if lives == 0:
            game_over_screen()
            start_screen() # Restart the game after its over
            score = 0
            lives = 3
            current_level = 1
        else:
    ...
    # Draw the platform
    pygame.draw.rect(screen, platform_color, (int(platform_pos[0]), int(platform_pos[1]), PLATAFORM_WIDTH, PLATAFORM_HEIGHT))
    ...
    #Draw the level indicator on a light-blue rectangle
    ...
    # Draw the lives in a red rectangle
